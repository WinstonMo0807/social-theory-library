...
export function WorkLibrary({ initialQuery = "", initialView = "all" }: { initialQuery?: string; initialView?: string }) {
  ...
  return <div className="admin-page work-library-page">
    <PageHeader eyebrow="馆藏" title={view === "editions" ? "出版版本与文件" : view === "quality" ? "需要检查的馆藏" : "馆藏管理"} description="查找文献，修改资料或处理文件。同一作品的不同出版版本可以分别查看。" actions={<><Link className="button secondary" href={withAdminReturn("/admin/cataloging/new", returnTo)}>新建书目</Link><Link className="button" href={withAdminReturn("/admin/uploads", returnTo)}>上传文件</Link></>} />
    ...
    <section className={styles.rows} aria-label={view === "editions" ? "出版版本列表" : "馆藏作品列表"}>
      {page?.results.map((work) => {
        ...
        return <article className={styles.row} key={work.id} data-record-id={work.id} data-row-type={work.row_type}>
          ...
          {work.health ? <div className={styles.details}><CatalogHealth value={work.health} /></div> : null}
          {work.row_type === "edition" ? <details className={styles.details} open><summary>版本文件与历史（{work.assets?.length ?? 0}）</summary><p className={styles.scope}>{work.current_reader_asset ? `当前阅读文件：${work.current_reader_asset.original_filename || kindLabels[work.current_reader_asset.kind] || work.current_reader_asset.kind}，文件版本${work.current_reader_asset.version}。` : "当前没有已选阅读文件。纯书目不需要虚构PDF或OCR进度。"} 替换和补充文件从“文件与阅读”进入；原件、历史及已有阅读页标识保留。</p>
      ...
      {page && !page.results.length ? <p className="admin-list-state">没有符合当前筛选的记录。可调整筛选或新建书目。</p> : null}
    </section>
