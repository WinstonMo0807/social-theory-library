...
export function KnowledgeWorkspaceDiagnostics() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const next = await apiRequest<KnowledgePayload>(`/catalog/admin/knowledge-workspace/?${params.toString()}`, { signal: controller.signal }, getServerSessionCredential());
      if (controller.signal.aborted) return;
      setPayload(next);
      setMessage("");
    } catch (error) {
      if (!controller.signal.aborted) setMessage(error instanceof Error ? error.message : "Knowledge Studio 读取失败。");
    } finally {
  ...
  return (
    <div className="admin-page knowledge-studio">
      <header className="admin-page-title">
        <div><p>系统诊断</p><h1>知识处理诊断</h1><span>用于排查来源、任务和索引问题。日常编目请返回知识策展页面。</span><Link className="button secondary" href="/admin/knowledge">返回知识策展</Link></div>
