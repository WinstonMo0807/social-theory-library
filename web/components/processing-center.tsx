...
export function ProcessingCenter() {
  ...
  const canRemoveRecords = hasAdminCapability(user, "can_edit_metadata");
  ...
  async function removeConfirmed() {
    ...
    try {
      ...
      setFeedback({ state: "success", message: `${label} 已从处理队列移除。可在回收站恢复，原文件和审计记录仍保留。`, actionKey });
  ...
  return (
    <div className="admin-page processing-center-page" aria-busy={loading || Boolean(pendingOperation)}>
      ...
      <ToastHost
        ...
        label="处理中心操作反馈"
      />
      <ConfirmDialog open={Boolean(removeTarget)} title={`移除“${removeTarget?.review_data?.title || removeTarget?.source_filename || "馆藏记录"}”`} description="这会把记录从处理中心和复核队列移除。NAS 原始 PDF、衍生文件和审计记录不会被物理删除。" confirmLabel="移入回收站" tone="danger" pending={actionPending} onCancel={() => setRemoveTarget(null)} onConfirm={() => void removeConfirmed()} />
    </div>
  );
}
