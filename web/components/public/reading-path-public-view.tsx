      ...
      <main className="page-shell theory-system-page theory-reading-path-page v307-knowledge">
        ...
        <section className="reading-path-stages" data-edit-section="paths">
          {path.items.length ? path.items.map((item, index) => <article key={item.id}>
            ...
            <div className="stage-copy"><p className="eyebrow">{item.stage_name}</p><h2>{item.work_data?.title || item.node_data?.canonical_name_zh || `第 ${index + 1} 阶段`}</h2><p>{item.stage_description}</p>{item.recommendation_reason ? <blockquote>{item.recommendation_reason}</blockquote> : null}{item.prerequisite ? <small>先修条件　{item.prerequisite}</small> : null}</div>
