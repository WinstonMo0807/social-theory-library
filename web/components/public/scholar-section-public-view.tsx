...
export function ScholarSectionPublicView({
  ...
  section: string;
  slug: string;
  relations?: ScholarRelation[];
  relationPagination?: ReactNode;
  ...
  const normalizedConcepts = knowledgeNodes.filter((node) => ["concept", "debate", "research_problem"].includes(node.node_type));
  ...
  return (
    <main className="page-shell secondary-detail-page v307-knowledge knowledge-section-page" data-public-page={section}>
      ...
      {section === "timeline" ? (
        <section className="panel timeline-detail-list" data-module-id="scholar-timeline" data-edit-section="timeline">
          {timeline.map(([year, event],index) => <article data-edit-row={index} key={`${year}-${event}`}><time>{year}</time><p>{event}</p></article>)}
      ...
      {section === "concepts" ? (
        <section className="panel definition-list" data-module-id="scholar-concepts" data-edit-section="concepts">
          {(curated.keyConcepts.length ? curated.keyConcepts : normalizedConcepts).map((item, index) => {
            ...
            return <article id={`concept-${index + 1}`} className="definition-row" key={`${label}-${index}`}><b>{String(index + 1).padStart(2, "0")}</b><strong>{label}</strong><p>{description || "说明待管理员补充。"}</p>{source ? <small>依据：{source}</small> : null}</article>;
          })}
