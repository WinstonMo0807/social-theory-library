...
export function AboutPublicView({ config, stats, about }: { config: SiteConfig; stats: SiteStats; about: { configured: boolean; blocks: AboutPageBlock[] } }) {
  ...
  return (
    <>
      <div className="page-shell about-page">
        <section className="about-hero" data-edit-section="about">
        ...
        {enabled("about-why") ? <section className="about-why" data-edit-section="about-why"><h2>{value("about-why", "title", config.about_why_title)}</h2><p>{value("about-why", "body", config.about_why_body)}</p></section> : null}
        <section className="about-principles">
          {features.filter((feature) => enabled(feature.key)).map((feature) => {
            const actionLabel = value(feature.key, "action_label", "了解更多");
            return (
              <article key={feature.key} data-edit-section={feature.key}>
        ...
        {enabled("about-process") ? (
          <section className="about-ingestion" data-edit-section="about-process">
        ...
        <section className="about-boundaries">
          {principles.filter(([key]) => enabled(key)).map(([key, title, body]) => <article key={key} data-edit-section={key}><h2>{value(key, "title", title)}</h2><p>{value(key, "body", body)}</p></article>)}
        </section>
        {enabled("about-notice") ? (
          <section className="about-warning" data-edit-section="about-notice">
