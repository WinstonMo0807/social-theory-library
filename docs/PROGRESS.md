...
已核对当前已部署3.0.5代码与原GitHub默认main的祖先关系，准备直接快进main，无新分支/无强推。README的旧2.8说明及GPT-HANDOFF首入口已修正，新增实际架构、上架过程、重设计任务和文档导航。事实已按前后台源码核对，包含Editor发布权、旧撤回权限差异、R2导入边界及Reader错误区分不足。本次不改业务代码、数据库或公网部署。最终推送结果见CURRENT_PROGRESS最新记录。
