...
## 2026-09-21 3.0.7 补漏与集中修复

已完整读取开发包与34张参考，并逐张复核。推荐/网站草稿、知识专用编辑与预览、13项平铺导航、5份本地上传、书目候选、SQL队列及兼容入口已实现；完整需求核对后追加共享原文策展和学者关系。首轮集中检查发现的问题已按影响范围修复复测，新增范围继续定向验证；日志和每次运行结果见[V3.0.7_COMPLETION_RELEASE](V3.0.7_COMPLETION_RELEASE.md)，不重复累加测试数量。新鲜备份和隔离恢复已成功。真实浏览器被本地Web启动策略阻断，尚未提交、推送或公网切换。
