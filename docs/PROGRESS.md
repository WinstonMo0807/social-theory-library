...
## 2026-09-24 3.0.8.1上线与验收

七项管理员易用性修复及19类可恢复删除已于NAS09-23 22:23:34上线，相关公网8项通过。正式镜像PG48项及此前分组检查通过；首轮版本常量遗漏导致自动回退，修正后重建API并成功切换。04:09 UTC用户原查询189段/11知识、Reader/PDF Range通过。OCR实际新任务保存2页，原290页已从8页恢复排队，其他5项暂停不变。09-24 12:21:44追加只读等待计时修复，仅更新API/Edge，不中断OCR；11项定向通过。准确源码、实际镜像、备份与限制见[交付记录](V3.0.8.1_COMPLETION_RELEASE.md)。浏览器豁免，召回质量和全场景性能不退步没有评估结论，OCR单页仍慢且整书未完成。

## 2026-09-23 3.0.8.1 实现与候选验证

七项管理员问题已落实到人物、封面、字段提示、回收站和学者展示的实际路径。新增可恢复删除并保留独立下架，修复长文本B-tree失败；OCR单页检查点、心跳、服务请求去重与繁忙重排队。检索新增保守关键词分支，限制多样性排序不提升低分尾部，减少重复材料读取。API预加载保留3个worker，推理保留准确权重和索引。NAS隔离PG41项、追加编辑25项、OCR/检索13项、OCR服务7项通过；本地类型、lint、构建和前述SSR已记录。真实浏览器启动再次被策略阻断，按授权跳过。性能对照及完整边界见[V3.0.8.1实施记录](V3.0.8.1_IMPLEMENTATION.md)。尚未完成生产切换。

## 2026-09-23 管理员反馈分析，尚未开发

完成本轮9张截图映射、相关源码和NAS只读检查，详见[分析报告](ADMIN_USABILITY_AUDIT_2026-09-23.md)。用户已确定上传及所有同类管理对象可恢复删除、删除与下架分开。人物回填/错误定位、封面保存边界、长文本B-tree和OCR恢复协议存在待修项。OCR最新为8/290页后再次失败；先前5页实跑记录保留为历史观察，不代表持续完成。此轮仅文档变更，无应用修改、测试通过、提交或部署声明。

## 2026-09-22 索引恢复与OCR直接控制

初始4483条文档/15来源已激活，公网新检索14项功能观察通过（报告输出异常单列）。OCR控制、断点手工重试和3.0.6布局已实现、测试，应用486af13已推送并于NAS22:31:39+08:00切换成功。最终候选PG9项、Web HTTP和部署后8项公网检查通过；无新迁移，保护摘要和活动索引前后相同。
...
## 2026-09-22 3.0.8应用切换与初始索引

应用e02b57013f已正常推送main，三种正式镜像构建、恢复副本迁移、新旧ORM与真实本地推理通过。NAS18:12:58+08:00正式切换退出0，无回退；4项增量迁移成功，保护摘要一致，四worker及API/Web就绪。23项公网原有业务检查通过；全库初始派生索引正在处理15个来源，完整新检索公网验收待索引激活后执行。历史Docker清理57容器/3网络已完成，其他项目与持久数据保留。详细独立运行结果见[交付证据](V3.0.8_COMPLETION_RELEASE.md)。

## 2026-09-22 3.0.8 集中验证

3.0.7上线与Git记录完成后才实施3.0.8。三通道检索、异步会话、上下文、索引/Processing Center、本地推理和增量迁移已实现。Django/迁移漂移/API契约、前端类型/正式构建、3项检索测试及2项SSR通过；前端lint错误已修，定向复核通过。NAS首轮83项中81通过，两项旧错误契约断言修正后仅定向复跑2项通过；新鲜备份恢复、增量迁移与新旧ORM保护摘要一致。真实模型与隔离完整检索链路仍在验收，未提交或切换3.0.8生产。公开检索不读私人编者备注，旧原文/阅读/人工观点与有效索引保留。各次独立运行与限制见[实现记录](V3.0.8_IMPLEMENTATION.md)。

## 2026-09-21 3.0.7 已提交并正式上线

已完整读取开发包与34张参考，并逐张复核。推荐/网站草稿、知识专用编辑与预览、13项平铺导航、5份本地上传、书目候选、SQL队列及兼容入口已实现；完整需求核对后补齐共享原文策展和学者关系。集中检查的问题已按影响范围修复复测，日志和每次运行结果见[V3.0.7_COMPLETION_RELEASE](V3.0.7_COMPLETION_RELEASE.md)，不重复累加测试数量。

应用提交`677058c79251c756a38764d334819ccd52424687`已推送原main，1282文件归档和API/Web镜像已完成。新鲜备份恢复、候选迁移/check、受保护摘要新旧ORM兼容和API HTTP就绪检查通过。NAS记录2026-09-21T07:42:20+08:00正式切换退出0，生产4个迁移成功，公网ready为3.0.7、数据库正常、待迁移0；Web/两Worker就绪、受保护摘要一致、cloudflared未变。公网最小探针21/21通过，退出0、37.049秒，覆盖7个HTML、544页Reader、Range206/PDF头、题名/原文搜索与定位及5个匿名私人接口401。源码树、归档SHA、镜像ID与不同主机时间戳见[部署说明](DEPLOYMENT.md)。用户已明确豁免受本地Web运行策略阻断的真实浏览器验收，未将该项记为通过；真实外部模型和OCR未复验。


## 2026-09-20 3.0.6 main同步与AI交接

已完成正常推送：199a7a4ebdedecfdb4999ae9951d19bf527af20f包含全部本轮功能及架构说明，远端main与本地一致，公开GitHub上管理端画像blob核对相同。1198文件安全扫描、10份入口文档151链接/43源码路径、43路由一一映射检查通过。此次没有新增业务验收或再部署；本段属于事后文档记录。

用户追加授权将已上线3.0.6直接同步原main。重新核对当前43个page、六组菜单、32条公开页面/能力契约、真实保存/权限/时间线/封面/OCR入口；新增ADMIN_ARCHITECTURE_PROFILE，更新架构入口、上架流程、重设计要求、当前状态和导航，旧阶段材料明确标为历史。只读生产确认镜像ID、重启次数、Edge模板及公网ready；没有再次部署或修改生产。源归档对照显示运行代码内容一致，Git工作文件换行差异单列。文档检查脚本只查引用与入口覆盖，不冒充业务验收；Git实际结果以CURRENT_PROGRESS顶部和远端main为准。

## 2026-09-20 最终上线

3.0.6基础8项补漏与封面追加已部署，应用源59e73c4108，Edge上传模板另按SHA独立校验。封面16项真实PG全部通过/109.259秒；本地16项API、真实封面操作、390/1280及PDF三态五宽度、2项网关契约、构建/类型/迁移检查通过。公网17项探针和两宽度Reader画布/翻页通过。备份实际恢复、旧版兼容及受保护数据前后摘要通过；完整记录见V3.0.6_COMPLETION_RELEASE/DEPLOYMENT。真实外部OCR/模型与全量验收未重跑，LoRA延期；不影响本轮按最新授权完成的最小发布。下方为过程时点。

## 2026-09-20 基础3.0.6已上线，封面追加更新准备

基础8项已部署冻结树8fae6082c1，备份恢复、迁移、旧版ORM、受保护数据摘要、公网页面/Reader Range/语义及私人权限检查通过。用户新增封面已实现并通过16项SQLite接口、1条完整浏览器选择流程及窄屏/宽屏检查，另三态PDF五宽度回归通过；实际图片预览重复/api问题修复。真实候选镜像的PG回归正在复跑流式响应测试的事务边界，尚未切换封面补充。参见V3.0.6_COMPLETION_RELEASE和CURRENT_PROGRESS；本段取代下方“未部署”等早期时点表述，不将历史测试累加为整版验收。

## 2026-09-20 补漏开发和最小验证

8项源码回溯缺口已完成，48项PG接口、20项前端单测、3条浏览器流程通过；推荐时间格式缺陷修复后3项聚焦API及浏览器通过。备份已创建并恢复到隔离库，当前镜像构建/迁移回退准备中，尚未宣称公网已更新。见V3.0.6_COMPLETION_RELEASE.md。最新用户授权取代下方禁止测试/部署安排，历史全量结果不重复计入本轮。

## 2026-09-20 剩余开发项及OCR呈现交付（未测试）

本次报告中的开发缺项已补入代码：处理中心普通/语义任务在SQL合并前做全范围筛选和计数、合并稳定排序后分页，不再限取300条；人工待办/上传记录提供翻页，筛选及页码保留在URL。OCR使用当前Work/Edition名称及出版信息，查询同样包含已保存未发布名称；来源文件和技术ID仍可展开追溯。默认以完成/总页数、剩余量、最近更新、原因及当前可执行操作呈现，页码显示范围并折叠，不删除页面内容；继续、取消、结束旧任务、全库暂停各自解释影响。OCR独立进度查询仍只读取原任务，不触发收费检查。

学者别名/机构、理论别名/分类及阅读路径建议改为表单预填和撤销，显式整页保存才写入。候选权限、来源、时效、身份、当前值和冲突由后端继续约束；表单与采用记录同事务失败回滚。学者译名的语言、类型和来源通过原PersonNameVariant/EditorialRevision补齐，不只写一个别名字符串。被管理员移除的预填不标为采用，也不因其来源过期阻止另行手工保存。单独的关联/观点操作不自动保存整页，未保存输入保护及超时/已存未读反馈接入。学科/子学科切换与离开保护、图片失败时保留已保存文字及版本、主题/学者离开保护、保存范围/时间/停留位置和预览/发布含义已补齐，子学科直接使用已有预览发布面板。

用户本轮取消所有测试、构建、浏览器、验收和部署检查，实际未执行这些操作，LoRA未展开；没有新表或migration、删除数据、连接生产、提交/推送或部署。现有清单继续区分历史结果与当前未经测试的改动，不新增平行public control登记。源码开发清单交付不代表3.0.6整版验收或生产可用性通过。

## 2026-09-20 字段联动及实际填写继续实施（未测试）

原字段助手已接本页未保存的公开编目信息，依赖指纹来自现有字段策略；不将未保存值当正式数据。已有结果只读，明确点击才允许外部查找，已有ResearchRun结果自动回到对应字段。作品/版本的普通标量字段扩展到同一个整页保存事务和来源/最终人工值记录，人工锁仍由后端执行。新对话框复用原生Dialog，减少字段旁浮层错位；知识名称只填明确字段，不隐式保存、刷新或覆盖生卒年。关联/观点需写库时先显示保存范围，已保存但读取失败独立反馈。子学科候选改用真实Subdiscipline关系而非旧KnowledgeNode/parent，右栏发布提醒去重且完整问题可展开。

全部为源码实现，按最新用户指令没有运行测试、类型检查、构建或浏览器。OCR新能力也仍未验证。原43入口/F01-F12/A01-A32、Reader与私人数据、外部服务、恢复/部署仍不减项；其他后台易用性最后收尾尚未完成，不能宣布全部功能完成或开始提前测试。无新migration、训练、外传或生产操作。

## 2026-09-20 时间线出处收口与新增全本OCR开发

时间线具体出处作品/Edition/阅读文件/页序已经接通，保存/发布均校验，公开链接依赖原有效修订资格；最终147相关API、242前端和44隔离浏览器通过，详见验证账本。该结果早于新增OCR代码。

新增加的已发布/未发布馆藏重新OCR及处理中心选书入口已写入：复用现有任务、服务和公开更新链，逐页真实计数，独立轮询、阶段/停滞/失败反馈、幂等、恢复和历史分页。原页标识和已公开文件不重建；临时请求仅发送当前页且映射原页序。用户随后明确要求所有实际功能开发完才统一测试，现已停止新增测试和验证性构建；OCR及其后改动全部待验证。未上线3.0.6，原全部范围不减少。

## 2026-09-20 01:41 身份建议与表单布局增量验证

作者、译者、出版社复用现有字段助手进入实际表单，保留原作者和职责，撤销只撤新增选择，整页保存再校验候选/身份/人工锁。未知姓名进入新建框，不自动建人或合并；原来源及最终人工值留在既有记录。多作者使用结果按具体身份记录，不能把名单组合变化算人工改写。目视发现的窄屏竖排/宽屏半宽表单经真实红测修复，移动目录焦点避开固定栏；没有隐藏内容或新增平行界面。

最新96 API、242前端单测、tsc/构建均通过退出0；扩大浏览器27项26通过/1测试准备错误，同业务源码下纠正焦点准备后4项填写专项全通过，五视口/键盘和截图复核完成。两组去重27个不同场景，不虚增为30，失败记录保留。精确命令及证据见V3.0.6_VERIFICATION与output/playwright/v306-identity-verified-0140。无新增migration或生产操作。完整联动、时间线文件出处、剩余易用性和原整版/真实环境/部署门槛继续保留，下方“身份类尚未预填”为历史状态。

## 2026-09-20 本次保存与智能填写增量验证完成

整页事务保存、简介/年份可见预填与撤销、理论流派次级时间线已完成本轮验证：最终107项API、242项前端单测、18项真实隔离浏览器全通过，均退出0，tsc/构建退出0。证据详见V3.0.6_VERIFICATION最新节与output/playwright/v306-workspace-verified-0032。新增多版本红测真实复现共享Work草稿被另一个Edition覆盖，原修订服务和新旧写入入口已补409保护。候选来源、原值、建议值和人工最终值沿用现有记录；没有训练导出、数据表新增或生产操作。其他身份字段预填、完整联动、时间线文件证据选择及原整版验收仍未关闭，未提交/推送/部署3.0.6。下方记录保留各阶段当时状态。

## 2026-09-19 书目整页保存与理论次级时间线

用户授权继续实施并补充长期训练数据准备。已实现Edition级整页事务保存/请求回执/共同编辑版本；简介和出版年份先填入输入框、可人工改写再显式保存，保留原建议与最终值，采用不计为正确性或训练许可。重复分节保存按钮移除，确认仍保留且不强迫跳转。理论下接入section=timeline，按node筛选并预选新事件关联，旧全库路径和既有公开读取不删除；43个page.tsx数量不变。新API11项通过退出0，扩大67项66通过/1fixture错误后已修；构建/tsc通过。真实浏览器和前端单测最终复验中，身份类助手全量预填、文件证据选择、原整版门槛未关闭。无迁移新增，无提交/推送/生产切换。

## 2026-09-19 3.0.6 专门编辑页补齐

22:16：关系/时间线完整表单与三类35条分页5项浏览器通过，退出0；242前端测试报告全过但旧进程退出码未知。新源94072fee真实PG33项通过、0跳过/退出0，验证0055旧ORM和真实行锁。隔离API/Worker/Beat已刷新此源，合成DB快照和旧容器保留，迁移/check/HTTP ready通过，无生产变更。额外真实时间线红测复现保存重设排列，已保留未编辑属性；当前API和浏览器全量、4条真实服务流程重新执行。此前全量中断记录不能当通过。

21:35恢复核对：扩大88项API全通过、0跳过，68.919秒；专门页双账户浏览器2项通过、78.329秒，产物已归档v306-relation-timeline-retry-0919。两进程旧句柄失效，退出码未知。目视发现360px时间线预览仍撑宽，按钮测试未覆盖整张表单，正在补测并修；两专门页无列表翻页也仍需处理。0055未更新到NAS，生产未改。

关系与时间线的6项红测确认缺少修改版本及保存直接影响公开内容；修后51项API组合通过退出0，原失败证据保留。两类对象纳入既有EditorialRevision，0055仅增加对象类型choices，无数据重建。专门页明确保存/正式发布、保留未保存输入和同对象冲突恢复；时间线保存保留旧关联来源说明。当前正在补浏览器实际操作及扩大权限回归，不能据此宣布R38/R39整体关闭。普通Editor独立发布能力不变，未切换生产。

## 2026-09-17 3.0.6 真实服务门槛复验

23:14：最新真实服务手工发布、同版本补充/替换PDF两项通过，退出0，证据v306-services-context-final-2314；替换验证了实际新Range、旧文件历史与私人Page关系。A06上传及两个兼容入口query保留见v306-services-context-2306通过项，不掩盖该组旧Range切换竞态失败。三种PDF状态×五宽度实页通过，退出0，v306-file-body-five-widths-2314。API当前修复56项通过，完整1416项（1376通过/40跳过）早于这些文件上下文修改；Web242项、tsc及构建通过。合成备份已恢复到独立空PG库，11类记录摘要、7原件、21页及私人记录一致，8公开详情/2Range通过；不是生产备份或全NAS灾难恢复。关系/时间线编辑、完整交付表和新鲜生产备份仍未关闭，未提交、推送或生产切换。

01:30：可选语义索引未配置阻止有效PDF公开的问题已修。同一发布服务分别读取公开书目和全文的真实完成记录，保留可选失败与旧公开快照、严格校验来源及阅读文件，不改任务成功标记。第一组合28通过/2个PG跳过（118.715秒），扩大54通过/0跳过，均退出0；原9项红测5失败证据保留。NAS隔离源570文件SHA256 2ae0d6b8b70f08ca0ce337cbe7ee8fefd065323f88f2b25173f990bccefea687，API/Worker/Beat已更新并check通过，完整页面服务演练开始。无迁移、提交、推送或生产切换。

## 2026-09-16 3.0.6 实施中，尚未部署

19:49：编目/人物/媒体20项修后通过，目录五宽度截图已补并归档。实际PDF补充演练发现文件区错误确认按钮提交400，已去除并修正保存快捷键，构建与tsc通过，演练继续。上传采用现有R2协议，隔离MinIO仅作外部存储替身，真实导入仍经PG/Redis/Worker。该测试不验证外部R2、OCR或模型，生产未切换。

14:55：作者/译者刷新竞态修后API组合38项通过，慢加载/刷新失败浏览器保护通过。最新编目组合19/20通过，图片回执被刷新提示覆盖已修待重验。真实隔离PG/Redis/Worker上的Editor纯书目发布A05通过，非模拟发布状态；A06上传PDF演练运行中。目录低对比与长标题修后五宽度测量通过，截图归档失误需补图。原整版验收与生产发布仍未完成。

14:24增量：旧编目/人物/媒体/肖像18项浏览器通过，退出0/184.581秒；阅读路径发布后图片状态刷新已修。真实服务上架发现字段助手新建作者与随后确认本节之间的旧表单覆盖，两项API及两项浏览器红测已复现，正在补版本校验与刷新失败保护。新增API改动需重新验证，不能沿用前1399项结果宣布本版通过。

本轮依据43后台入口、公开读取登记与F01-F12实施共同书目状态、全来源待办、服务端分页、版本文件操作、阅读器/二级页布局和后台精简。精确阶段结果以CURRENT_PROGRESS及V3.0.6_VERIFICATION为准，以下3.0.5段落是历史上线记录，不是3.0.6结果。

最新完整API为1359通过/40跳过，退出0；真实PG原76项72通过/4Worker启动错误，测试broker环境修后4项Redis/Celery恢复独立通过。六类知识编辑双账户保护、43入口五视口、推荐和Reader扩查的完整浏览器119项全部通过（584.269秒，完整XML和日志保留；恢复后旧句柄失效，退出码待核实）。最新前端241项单测和构建通过。现在补齐普通Editor通过真实隔离PG/Redis/Worker/Meilisearch进行上架及文件维护，未直接设置任务成功。当前无新增migration，无3.0.6生产切换、提交或推送；原A01-A32及新鲜备份恢复仍是部署门槛。

## 2026-09-13仓库默认分支与重设计上下文

已将当前已部署3.0.5代码及上下文提交bf5edb0直接快进并推送main，无新分支/无强推，保留历史。GitHub提交、1098文件清单和6份入口文档blob与本地一致，仓库保持PUBLIC。README的旧2.8说明及GPT-HANDOFF首入口已修正，新增实际架构、上架过程、重设计任务和文档导航。事实按前后台源码核对，包含Editor发布权、旧撤回权限差异、R2导入边界及Reader错误区分不足。本次不改业务代码、数据库或公网部署，核验记录提交仍只更新文档。

## 2026-09-06 3.0.5 开发中

2026-09-13本次交付已上线。API源a829bb8，Web源fa6b3bc；最终代码与运行镜像、备份回退和功能边界已记录在CURRENT_STATE/DEPLOYMENT/UI_3.0.5。用户报告的旧PDF发布不显示已解决，后台ready/ready/published、event completed、变化为空，公网7本与实际PDF阅读通过。应用代码本地提交，未推送。
2026-09-13最终源代码收尾。3.0.5已上线并恢复目标书，公开搜索/详情/Reader画布和PDF Range通过。发布完成阶段的PG可空JOIN锁补丁已在真实恢复副本先红后绿。CSS按职责拆为67个文件、11类token、189处等值引用；server-api只保留兼容导出，101声明及45消费者正文等价，客户端Reader不再导入服务端transport。统一组件已接实际页面。完整前端构建与212项通过，退出0，35.791秒。正在冻结最终候选并更新公网，没有再次改schema或重建活动索引。
2026-09-12 22:44生产备份实际恢复完成，候选补0054数据库默认值以保持旧API回退后的新增书目能力。专项2项通过，退出0，45.94秒，Django和migration drift通过。原候选未上线，现重新冻结构建。NAS网络下载缓慢，API保留核验一致的原运行依赖并加入8个哈希固定schema依赖，Web按新锁文件离线npm ci；镜像来源和包哈希另存发布记录，尚未生产迁移或公网切换。

2026-09-12新增生产发布排查已获用户授权部署。当前3.0.4的《质的研究方法与社会科学研究》虽标published，但没有活动公开修订，knowledge_graph/person_search持续等待并阻止书目公开。真实状态在只读事务核对，公开仅6本。修复将保持原件/正文资格、允许已完成书目要求的metadata-only首发公开，并保留派生任务等待状态。备份780098c8-17c1-4090-a52e-4e00cb14090d已完成且独立PG16实际恢复成功，尚未切换生产。

checkpoint5的原12失败已按缓存隔离、肖像草稿契约与公开媒体字段登记作最小修正，19项定向通过，退出0，61.96秒。依赖补丁更新后critical归零，剩image-size/Vinext构建路径2high与Drizzle开发工具4moderate均保留显式记录；并非全部安全扫描归零。当前继续发布问题和原规格中编目会话、字段投影单一来源、Reader模块及标识符收口，不重新跑多轮全量测试。

知识节点/阅读路径图片与加载保护于2026-09-12保存为本地提交f710492，未推送。当前开始f710492上的完整后端checkpoint5，测试时冻结应用与测试源码，仅允许记录更新。整个3.0.5和生产门槛尚未完成。

2026-09-12 知识节点/阅读路径图片与编辑保护切片本地收口。后端68项、完整前端198项和构建通过；完整浏览器18项通过，退出0，98.609205秒。最后横幅填满与390px复核3项报告通过，63.258111秒，报告在12:45:53完整写入，恢复后退出码未知。桌面与移动截图已检查。当前准备本地提交，未推送、未应用0053生产迁移。不会将本地验收当作真实NAS、公网或索引可用。

2026-09-12 理论节点和阅读路径图片完整专项3场景通过，退出0，65.573231秒。包含指定节点加载期间禁止输入、说明草稿与图片同发布、公开图片真实加载、清除后旧原图保留、阅读阶段原ID不变。后端7文件68项、完整前端198项通过，仍在扩大原15场景的整体回归。未提交本切片、未执行0053生产迁移或部署。

2026-09-12 KnowledgeNode/ReadingPath媒体选择已实现初版，0053增加两个可空图片FK，复用EditorialRevisionMedia和共享组合草稿。图片/元数据/阅读阶段不互相替代，发布前图片只向后台开放，公开读取检查既有status与功能开关。后端扩大53项、完整前端198项及构建通过，浏览器验收仍在运行。本切片未提交或生产应用。

学者肖像切片已于2026-09-12保存为本地提交0e52c18，源码与凭据模式扫描通过，未提交媒体、数据库或测试产物。未推送、未执行0052生产迁移。下一项为KnowledgeNode/ReadingPath的图片与编辑发布接入。

2026-09-12 学者肖像选图、草稿预览、明确发布、清除和历史图片保护已完成本地实现。后端6文件81通过，退出0，83.50秒，完整前端195项及构建通过，完整浏览器15场景通过，退出0，99.387109秒。含原编目、媒体、人物合并及新肖像从上传到清除的真实API操作。窄屏面板调整后额外1场景通过，退出0，63.533909秒，截图已检查，准备独立本地提交。未推送或生产迁移，0052仅在隔离测试库应用。

2026-09-12 学者肖像媒体选择正在实现。0052新增Person媒体FK和编辑修订的图片引用保护，采用既有媒体/草稿/发布服务。新图片、清除操作和兼容上传都先进入学者草稿，人物绑定变化或旧指纹拒绝覆盖。首轮31通过1失败为测试误认消费者名称，扩大72通过；中断的guarded无完整报告不计结果，重跑6文件80项通过，退出0，74.23秒。当前完整前端195项及构建通过，正在跑实际浏览器选图/发布/清除，尚未提交或生产迁移。

人物整理切片已在本地提交45a79fe保存，未推送。随后开始学者肖像的媒体接入审计，已发布肖像旧入口仍返回published_binary_requires_revision，不取消这条保护后直接写原文件；正式实现与验收尚未开始。

2026-09-11 人物整理管理页已接真实查找、查重、影响核对、Owner人工确认、记录重开及撤回。无独立ScholarProfile的人物同样可选，最近记录方便中断后核实，响应丢失时重用原请求。生产构建与独立SQLite的14个浏览器场景全部通过，退出0，83.619086秒，包含既有编目/媒体6项。完整前端192项通过，退出0。最后的列表摘要SQL优化及后端4文件51项通过，退出0，53.60秒，API漂移、Django/migration、TypeScript与lint通过；未推送或部署。范围仍限双方核验、无冲突、至多一份学者档案，不代表整个3.0.5完工。

2026-09-11 Person非冲突合并、操作详情与持久回滚API已实现，新增0051记录，仅Owner可确认执行。保留来源、引用ID和原审核信息，基础资料不自动覆盖，书目复用发布事件。最新6文件84项通过，退出0，71.16秒，包含文档文件、合并后私人笔记、恢复发布、迟到事件和事务失败。Django/migration/TypeScript/API契约与diff检查通过，按独立本地提交保存。首版仍拒绝双档案、唯一关系冲突和未处理编辑上下文；前端及PostgreSQL并发未完成，未推送或部署。

2026-09-11 人物查重与只读影响预览已实现。覆盖14类声明引用、身份与关联冲突、公开修订和草稿摘要、截断提示及活动词典词条。输入字段名不能成为查询操作符，缺失或错误词典状态不会被自动修复。最终人物/权限/契约/词典同步52项通过，退出0，46.16秒，生成OpenAPI/TS及检查通过。合并执行、持久回滚及前端尚未实现，不计完整人物合并功能。未推送或部署。

2026-09-10 完整后端checkpoint4报告1180项，1148 passed、0 failed/0 errors、32 skipped，728.984秒，23:31:43完成，对应源码bd81669。续接后进程句柄失效，退出码未知。31项PostgreSQL和1项大数据演练跳过不计通过。当前可运行后端回归已收口，Person合并及其他未完功能、真实服务与生产验收继续，不能据此称整个升级完成。

2026-09-10 剩余推荐/理论/段落索引回归收口。源码仅为 index_asset 增加修订归属和元数据就绪两项校验，最终6文件58通过，退出0，182.13秒，包含合法准备态独立索引范围、原文件/私人批注保留、替换/撤回/重新上架和投影。理论合并明确保留来源所有别名，仅复制正式已确认别名，回滚保留来源。没有真实索引或生产操作；随后进行完整后端checkpoint4，结果另记。

2026-09-10 命题/候选回归中确认观点基线、结果元数据及筛选项未严格跟随正式快照的缺陷。已增加活动文档、文件权限、真实原文、快照元数据和筛选复核，保留上一正式版本。分类 HTTP 入口也按正式关系解析。最终8文件103项通过，退出0，73.75秒，含26项新增边界用例。词典版本核对实际数据库状态，候选采用与实体正式发布分开测试。Django/migration/verified契约及diff检查通过，没有生产连接、索引写入或部署。

2026-09-10 OCR 分批、暂停恢复、选择性识别与文档智能四文件28项通过，退出0，49.22秒。只调整测试，验证未发布的 OCR 结果不进入正式发布、原件字节/人工页码保留、实际文档修订与证据，以及正式激活前和旧失效命题的公开拒绝。没有恢复已退役的同步索引调用，没有修改 OCR/公开资格实现。下一项为剩余命题、候选与词典回归，生产验证仍待完成。

2026-09-10 Reader 页码接口已分离元数据和正文可用状态。无正式全文的可读 PDF 仍提供页码与尺寸，但不返回未激活文字、章节和文本块。访问接口的 OCR 可用提示也检查正式全文，仍保留真实任务状态。最后 Reader/文件发布/公开修订/范围检索/阅读记录/入库集成 70 passed，退出0，148.07秒，包含私人书签/笔记保存及跨账号拒绝。封面通过真实维护发布 HTTP 验证，外部发布处理失败使用明确的 test-only 状态模拟，失败记录和原文件保留。Django check、migration drift、verified API 漂移通过。未推送或生产部署。

2026-09-10 最新完整后端 checkpoint3 为1063通过/38失败/32跳过，共1133项、583.194秒，应用源码4120c66。随后投影/语义版本/类型/范围组39项通过，知识实体发布/工作室/编辑修订30项通过，均退出0。本次修正测试与现有持久事件契约的对应关系，没有放宽失败、租约、权限或版本要求。全量仍未通过，待继续Reader和OCR等用例。

2026-09-10 推荐图例接入既有媒体原件/衍生图/资料快照和 Work 编辑修订，0050 仅增加可空 FK。替换/移除不删除历史文件，回滚保留精确图像引用，清除图例不改书封；图片更新只触发展示处理。后端最后扩大组 59 项、Web 完整构建/188 项通过，浏览器最后结果见 CURRENT_PROGRESS。没有生产迁移或部署。

2026-09-10 Reader/公开证据夹具补活动 Catalog/Document 修订，RAG 必须有正式全文，多文件快照严格使用指定 Reader Asset。修复旧封面选择先覆写 FK 再比较导致归属检查失效的问题。扩大 Reader/媒体/范围检索 62 项通过，退出 0，70.69 秒。仅本地，推荐图例旧写入口仍需统一发布处理。

2026-09-09 工作台回归改为核对实际字段决定、候选列表和正式发布前置条件。同名作品关联场景复现了人工关联后 OCR 重跑覆盖已有 Work 的真实风险；新增只读 effective_catalog_reconciliation，后台和流水线从当前有效人工选择取状态，原始机器报告不改。上传/会话关联与撤销、重跑保护等首组 39 项通过，含强 Edition 复用保护、智能编目与发布命令的最后扩大组 59 passed，退出 0，94.71 秒。Django check、migration drift、API 契约漂移均通过，无生产修改。

2026-09-09 旧元数据复核接入统一字段决定、人工锁与新增实体发布包，修复显式填写语言却被发布检查当作未确认的问题。未传语言不覆盖现有值，归档作者拒绝并回滚。与 Workbench 共用 DRF 契约验证，补 publication_date 保存与日期/年份一致性。最终复核/回填/完整入库 34 项通过，包含目录/搜索/引用/Range/替换文件/笔记/撤回/重新上架。消费者完成使用受控测试确认，实际 PostgreSQL/Redis/Meilisearch/模型未验收。

同次集成发现语义派发失败误改活动索引状态，已单独提交 8405d9f。12 项投递/持久任务恢复测试通过，active/retired 不变，任务失败及迟到任务保护保持显式记录。

2026-09-09 修复字段助手 `local_theory_school` 仍创建 WorkKnowledgeRelation 的真实旧写入。新选择复用受审查映射，只写规范关联或编辑修订，历史关系与证据不改。首组 37 项通过，包含 8 个新增安全/发布隔离/事务测试；补映射和目标锁后核心 23 项通过，Django check 与 migration drift 退出 0。无 schema 或生产数据变更，PostgreSQL 并发待核实。

2026-09-09 AdminShell 共享已验证会话：首页与统计页不再独立取认证或无条件请求管理员统计，所有摘要请求按账号隔离并可取消，队列错误不再显示为空。权限逻辑、未知状态与旧会话回退测试已补充，后端权限不变。最新完整前端构建/184 项通过，新增真实权限和断网重试场景结果见验证记录。

2026-09-09 补后台媒体封面草稿预览、原件/衍生图 ORM 内容保护和 0049 资料快照。只改图片说明或许可也可以准备、核对并发布新封面版本，已选资料不会被后续源编辑替换。当前 npm test 构建及 181 项通过，相关后端 48 项与真实 E2E 3 项通过，环境限制见 CURRENT_PROGRESS。

2026-09-08 媒体库与 Work 封面切片已实现，含 MediaAsset/Rendition、来源/许可/焦点、私有预览、响应式尺寸和发布引用保护。媒体选择进入书目草稿，公开仅从活动修订读取。36 项相关后端测试与 3 项真实浏览器测试通过，其他实体媒体与生产验证仍未完成，见 V3.0.5_MEDIA。

分层 readiness 已复用原探针系统，增加工作台构造、活动公开修订/serializer、迁移与公网 API 检查。`/ready` 明确只验证 DB/schema。只读 `check_functional_readiness` 不接受无样本的绿色结果，也不等同完整 release acceptance。探针/权限/调度 34 项、空样本 1 项、完整 Web build 与 181 项 Node 回归通过；生产尚未实测。

2026-09-07 继续回归治理。Reader/收藏/范围检索与 Range 27 项通过。前端旧工作台、知识诊断、控件语法和文案断言对齐后完整 `npm test`（包括新构建）181 项通过，TypeScript、lint 均通过。补回学者外部标识字段助手，保持显式采用。语义回归按活动 revision 建立测试数据并验证撤回 outbox，最后旧 fallback 断言保留空 Asset 范围约束，未降低访问过滤。
...
字段契约首轮已收敛当前工作台字段分组、必填、标签、依赖和可写字段。DRF 保存、字段助手标量采用和发布检查共用校验。0044 新增明确的纯书目模式，保留旧行 document 默认值。无 Asset 的纯书目可经原 outbox/revision 成为公开详情，不伪造正文和 Reader；存在坏 Asset 时仍阻断。该切片 50 项与后续 44 项相关检查通过，尚未生产发布。所有权威字段、外部 identifier 及投影细项的全量契约覆盖仍需后续完成。
...
15:03 用户发现共用工作台 500。已定位 7 处步骤漏传字段状态参数，最小修复提交 `ee89588` 于 15:14:25 完成 API 热修并推送仓库。上架、已发布维护、队列和字段研究共用该入口，因此并非单本数据异常。没有修改馆藏或执行测试，工作台尚未逐本验收。Cloudflare 隧道再次中断并恢复，长期稳定性另列问题。

3.0.4 部署记录位于 `storage/backups/pre-v304-cutover-20260905-134000/deploy-record`，已保留首次参数失败、旧应用恢复、第二次成功切换及 Cloudflare 隧道恢复过程。正式 source tree 为 `d38f23ba0b8b77a3df376b08d68abaaa85da7670`。后续仅文档提交不改变部署代码。

正式词典 registry v2 的 47 条旧来源问题已清除。最终生产审计仍有 5 项候选与确认关系差异、5 项关系端点警告，保留人工判断；13 类核心 ID 及 9 个 ORIGINAL 记录摘要均未改变。隔离恢复副本已清理，可由保留的正式备份恢复。用户暂停的 6 个 OCR 任务未恢复。

本任务历史显示于 14:31 修正字节位置和序号映射，恢复本任务 29 个回合与 1,918 条显示项，原对话未改写。按用户最新要求，修复以后没有运行测试。最终代码亦未追加功能、集成、E2E 或浏览器测试，A–J 仍为待核实。

下方保留实现阶段历史记录，其中的未构建、未迁移和未上线描述仅代表当时状态，不覆盖以上实际部署结果。

## 3.0.4 智能编目与知识发布

- 2026-09-05 发布前恢复审计发现已审核关系仍可能指向草稿子学科或归档节点。已补快照生成、0040 回填、公开运行时、全文/语义元数据及 Work/Node/Subdiscipline 公开字段的实体状态过滤。旧快照和后台关系保留不改，明确后台上下文仍可显示草稿；本次修复未测试、未构建、未上线，原历史关系警告继续保留人工处理。
...
- 生产 catalog head 已到 `0031_admin_workflow_v280`，pending migration 为 0。Work 6、Edition 6、Asset 12、Page 2,585、SemanticChunk 3,881，迁移前后 ID 集合哈希一致。生产 Reading Path、Stage 和 placement 均为 0，因此本次没有历史路径阶段需要人工合并。
- 活动语义索引保持 `semantic_passages_20260818210650_4cf87bc9`，数据库与 Meilisearch 都是 3,005 个 ready 文档。本次没有 reindex、ranking 调整、authority 自动发布或 Candidate 自动接受。
- 公网 ready、health、首页、Explore、登录、Admin、Work-centric Library、Intake Focus Mode、Maintenance Mode、旧 review/publication redirect、三条顺序观点检索和真实 PDF Range 206 已通过。Maintenance Mode 的策展与发布区只做只读检查，没有修改真实馆藏。
- 一个页面会话请求曾短暂返回 502，保留会话后重试成功。同期 ready、health、首页和匿名 auth 响应正常，API、Web 与 Edge 日志没有 500/502、Traceback 或重启。
- 入库仍有一个明确未解决的生产问题。UploadItem `92ea5bc2-904b-49cd-848f-67accff9639d` 处于 R2 `uploaded`、本地 file 为空，关联 ProcessingJob `ca68dde3-f1b4-4e0b-872d-45fc1e3ba261` 保持 pending。Beat 每分钟恢复时分别触发 file 缺失和 nullable outer join `FOR UPDATE` 错误。本轮依照用户要求只记录，不修改任务、数据库或 R2 object。
- 远端发布暂存、专用 sudoers 和本轮两条公钥记录已删除；本地临时密钥也已删除。撤权后的 ready、health、Maintenance Mode、V2 semantic 和 PDF Range 206 继续通过。
...
- 部署前曾在独立本地 SQLite 从零应用 catalog 0031，readiness 为 2.8.0、pending migrations 0。本地预览通过 `http://localhost:3100` 提供。该结果只属于本地门槛，生产结论以上一节的实时证据为准。
...
- 新镜像 migration plan 只有 `reading.0007_reader_ai_connection`。Worker 和 Beat 在空队列窗口停止，migration 用时 6 秒并通过 Django check。最终 API、Worker、Ingestion Worker、Beat 和 Web 使用统一 `2.7-87251cb` image family，Edge 已刷新。
