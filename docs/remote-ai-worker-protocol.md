...
3.0.2 支持两个有界任务。`claim_extraction` 读取单个 EvidenceSpan，完成后写入 `DerivedClaim` 与 `ClaimEvidence`。`library_synthesis` 只读取两至十二条馆内 EvidenceEnvelope，完成后写入可审核的 EnrichmentCandidate。两类结果都属于派生或候选数据，不能直接修改 Canonical Knowledge。只有专业结果成功持久化后，CapabilityDemand 才会成为 completed。

## 3.0.2 可运行客户端

仓库已提供 `api/common/remote_worker_client.py`。它是可直接运行的 pull worker。客户端先发送 heartbeat，再领取已明确声明的任务，调用笔记本本地的 Ollama、vLLM 或 OpenAI-compatible endpoint，最后通过带租约的 completion API 写回派生结果或候选。客户端不能访问 PostgreSQL，也不能直接写 Canonical Knowledge。
...
# 默认仅启用 claim_extraction。下列两行保持省略即可。
# 只有私有环境完成模型质量和资源验证后，才可显式增加 Library Synthesis。
# STL_WORKER_CAPABILITIES=llm_small,llm_large
# STL_WORKER_TASK_KINDS=claim_extraction,library_synthesis
...
首次接入可使用 `--once`，它只 heartbeat 并最多处理一项需求。服务端 URL 在非本机环境必须使用 HTTPS。笔记本下线后 heartbeat 会过期，未领取任务回到 `waiting_for_capability`；已过期租约不能提交结果，人工上传和发布不受影响。3.0.2 公网发布不包含 4070 客户端部署，协议可用不等于笔记本已在线。
...
Worker 下线后，heartbeat 到期和租约回收由 capability reconciliation 处理。没有合适执行器时，Claim 与 Library Synthesis 任务保持 `waiting_for_capability`，且 `publication_blocking=false`。过期、来源 revision 已变化或目标已失效的需求由 reconciliation 标为 superseded，不应长期占据有效 backlog。

## 请求边界

所有端点只接收 JSON，并受 `CAPABILITY_REMOTE_WORKER_MAX_REQUEST_BYTES` 与专用 API rate limit 限制。馆藏原文响应受 `CAPABILITY_REMOTE_WORKER_MAX_JOB_TEXT_CHARS` 限制。Claim 结果最多包含 24 条候选。Library Synthesis 最多引用 12 条 EvidenceSpan，引用标识必须来自租约内的 EvidencePack。服务端不接受 PDF、模型文件或任意二进制上传。
