...
CSS结构/语义摘要、直接API领域边界及原反馈检查共16项通过。最终统一构建和212项前端检查通过，退出0，35.791秒。公网菜单为真实modal，焦点进入关闭按钮、背景不可交互，Escape关闭后焦点回菜单按钮。APA后按右方向键选中并聚焦Chicago，tab入口只有1个。390px文档宽度390，截图已查看。真实Reader已渲染PDF画布。没有将未登录的后台写入测试写成通过。
