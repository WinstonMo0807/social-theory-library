...
- 2026-09-11 Person查重和只读影响预览已实现，相关52项通过，但仍为`merge_execution_available=false`。实际活动词典影响、未知/截断提示及标识符输入边界已核对。两个ScholarProfile、通用候选JSON、人工锁定字段、正式发布与回滚尚需执行器和前端完整处理。沿用Owner-only合并权限，未新增写入口或修改人物/历史数据。
