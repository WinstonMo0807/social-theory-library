...
## 2026-09-24 3.0.8.1已修复及剩余边界

七项易用性与长文本B-tree修复已上线，新增回收站与下架区分；正式PG48项、公网8项通过。原失败OCR已恢复排队并保留8页；另一个新版新建任务确实保存2页文字/文字块，当前仍有较慢页面。持续心跳和服务running已可见，但不等于整书完成，不能宣称OCR吞吐已提高。排队计时沿用旧阶段的实跑缺陷已单独修复，11项定向通过，API上线且Worker未中断。

剩余限制：真实浏览器因执行策略拒绝按授权豁免；正常登录后的完整管理员操作与视口对照未执行。API有限对照内存下降约12%，冷态首轮没有提速；Inference未证明内存降低。NAS N5105/8GB存在实际CPU与换页压力，本版不扩模型或降低精度/并发。检索没有人工标注质量基准，公开原查询有结果不等于召回质量已证明改善。具体实测及镜像见[发布记录](V3.0.8.1_COMPLETION_RELEASE.md)。下文“待修”“尚未上线”等保留为历史时点。

## 2026-09-23 3.0.8.1 候选状态

下节只读问题已形成代码修复并通过相应局部及NAS隔离检查，尚未上线。真实290页OCR后续进度、最终公网检索仍需切换后验证。浏览器启动被自动审批阻断，已按用户授权豁免，不作为视觉验收通过。API隔离内存降低约12%，同并发热态样本改善；推理结果与旧权重逐值相同、速度样本改善但内存未明显降低。NAS N5105/8GB存在换页压力，本版不扩大模型；无人工标注召回基准，不能声明召回率已提高或所有负载下绝无退步。

## 2026-09-23 管理员实测新增待修项

本轮只读核实，尚未修复。原290页OCR最新在8页后failed/max_attempts_exhausted，运行恢复门槛30分钟但HTTP等待60分钟，识别期间缺少心跳；NAS有CPU及换页压力。图1另为Passage.normalized_text普通B-tree索引项超长，不应归因为PDF损坏。作者/译者新建追加行及错误状态不刷新、封面整页保存依赖/指纹失效风险、删除与下架语义不一致、学者原名及长简介布局均列入下一版。封面历史失败尚无原始HTTP响应，不能把风险推断写成已复现。详见[完整分析与验收范围](ADMIN_USABILITY_AUDIT_2026-09-23.md)。当前未改应用或生产状态。

## 2026-09-22 用户报告的观点空结果已恢复

根因为第一次完整discovery索引未激活就切换公开入口。现全部15来源/4483文档激活，用户复现查询completed并有原文定位，非PDF丢失。普通及扩大查询、权限、上下文、分页、取消已实际通过；NAS后台单次复现18秒，不宣称性能或相关性评估已完成。初次构建较慢与入口切换顺序分别记录。

OCR旧失败任务3次自动尝试已用尽；NAS深度模型探针及失败批次单页HTTP成功，旧500具体堆栈无法追溯。新的手工重试、2秒真实页进度与暂停/继续已上线，重试保留旧错误审计并开启新一轮有限尝试。
...
切换发现一条2026-08-19起未更新的历史BackupJob `baf72116-6534-4e05-9821-e0526060fa39`仍标running。四Worker active/reserved/scheduled、broker队列/unacked均0，宿主无备份进程。记录未删除或改写；发布首轮静默检查因此退出1并恢复Beat，随后仅按该准确ID、状态和时间戳在只读检查中单列历史残留，其他运行门槛保留。该历史状态残留未修复，不影响本次新鲜备份完成与校验。

## 2026-09-22 3.0.8当前验收边界

正式应用及追加API/Web修复已上线。初始全部15来源/4483文档已激活，公网新查询及上下文/分页/扩展/取消有实际证据。本批8项公网检查通过。单次用户复现后台18秒、部署后另一查询公网轮询34.41秒，仅为各自样本；相关性基准和持续性能评估未完成。

真实浏览器按用户明确指令豁免，未标记视觉验收通过；没有合适真实繁体馆藏样本，模型级繁简验证不替代该项。当前没有公开策展内容，空通道是实际数据状态；隔离且明确标记的策展样本已验证索引、公开与撤下联动。NAS清理中废弃历史测试PG使用tmpfs，停止后内容释放且没有有效另行冷备；仅该废弃测试库受影响，生产、本次备份/恢复和所有持久卷保留。详见[交付证据](V3.0.8_COMPLETION_RELEASE.md)。

## 2026-09-22 3.0.8 早期验证边界（历史）

新检索已进入集中验证，构建、类型、Django、API契约与风险测试已实际执行；不能据此证明真实模型或生产索引可用。固定两模型约976MB及Linux离线依赖已下载校验，NAS N5105无AVX/AVX2，真实编码及重排待验证。只读复核3.0.7服务正常；用户新增授权清理书库历史隔离开发容器，严格保留其他项目及生产数据。清理后的内存余量须重新实测，后续重操作串行并观察RSS。真实浏览器此前运行策略阻断已获用户豁免，仍不标记视觉验收通过。

## 2026-09-21 3.0.7 当前边界

用户已明确豁免受本地Web启动策略阻断的真实浏览器验收并授权部署。该项是已接受的验证限制，不再阻断发布，其他检查和回退要求不变。应用提交`677058c79251c756a38764d334819ccd52424687`已推送，并在NAS记录的2026-09-21T07:42:20+08:00完成正式切换，退出0、未回退。生产迁移、API/Web/Worker就绪、受保护摘要一致性及公网ready通过；公网最小业务探针21/21通过，退出0，用时37.049秒。

真实浏览器限制：原Web启动命令在本轮由同一执行工具重提一次，仍返回CreateProcess Rejected / blocked by policy。拒绝发生在应用启动前，未提供具体原因，不能断言来自某一自动审核器。当前完整访问和never审批状态没有可用审批项；没有改写权限、换壳或改用其他服务规避。3.0.8尚未开发，也未读取其开发包。

整合发现的计划项版次关联、选图旧输入覆盖、媒体分页、初次知识草稿发布、处理中心分页任务及缺少共享原文/关系身份已修复并随本次上线，检查按实际运行分别记录。真实交互、截图对照及外部模型/OCR复验未执行，不记作通过。公开推荐与共享关系接口为合法空集；Beat定时健康任务恢复后存在短暂队列消息，业务open为0，不把维护消息误判为业务阻断。详见[V3.0.7_COMPLETION_RELEASE](V3.0.7_COMPLETION_RELEASE.md)与[部署说明](DEPLOYMENT.md)。
