...
- 2026-09-11 Person非冲突执行与持久回滚API已接入，最新84项通过。仅完整、已核验且无阻断的执行预览可以确认合并；双档案、重复唯一关系、待处理上下文仍拒绝，不自动取舍。新增0051未在生产应用，前端与PostgreSQL并发/真实索引消费仍待验证。不能将可执行子集称为任意人物均可合并。
