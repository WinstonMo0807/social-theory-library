...
- 续接、中断恢复或额度恢复后，先读根目录 `CURRENT_PROGRESS.md`，核对当前 Git 状态与最后修改点，并先向用户说明已完成、未完成和当前动作。不要仅从聊天最后一条或旧阶段文档猜测进度。
- 每完成一个可交接的小项，及时更新 `CURRENT_PROGRESS.md`，再报告完成。保持日期、当前动作、剩余项和提交/部署状态准确。实现、已验证和已上线分别记录。会话中断后只核对最后一项，不重复已完成的全量审计。
