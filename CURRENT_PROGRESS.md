...
下一项是 OCR 分批、暂停恢复和选择性识别测试中的已退役同步索引入口。最新完整后端仍为 checkpoint3 的1063通过/38失败/32跳过，不据这次70项推算全量成绩。Person 合并、其他实体媒体、完整 API/Reader/界面/legacy 收敛、真实 PostgreSQL/索引/Provider 和生产恢复验收仍未完成。
