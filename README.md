...
当前源码版本为 **3.0.6**，已于2026-09-20完成公网切换，包含管理工作页、智能填写、当前版本重新OCR及封面联动。仓库默认分支 `main` 提供当前代码与后续管理端重设计基线。运行镜像来自上线时冻结的源码树，另有Edge封面上传配置增量；本次Git提交不是原镜像构建标签，确切对应见[当前状态](docs/CURRENT_STATE.md)和[部署记录](docs/DEPLOYMENT.md)。记录是有日期的快照，不保证未来实时服务状态。
...
用户需要进一步大幅改进管理端易用性。本次提交包含已经实现并上线的3.0.6，不是只有文档；下一版方案尚未确定。可以删改冗余界面，但须说明替代、数据保护与兼容条件，不擅自换栈、删原件或新建第二套书库。先说明用户操作和数据变化，再给代码与迁移方案。
