...
EXPECTED_VERSION="${EXPECTED_VERSION:-2.6.0}"
PREWARM_SEMANTIC_MODEL="${PREWARM_SEMANTIC_MODEL:-1}"
PRELOAD_OCR_STRUCTURE="${PRELOAD_OCR_STRUCTURE:-0}"
REQUIRE_OCR_MODEL_READY="${REQUIRE_OCR_MODEL_READY:-1}"
...
HF_MODEL_ARCHIVE="${HF_MODEL_ARCHIVE:-${SOURCE_DIR}/offline/social-theory-library-huggingface-model-2.6.0-e8f8c211.tar.gz}"
HF_MODEL_SHA256="1df537373de4b09ef4c0c6c70b5b2e8c9f245e15657c99f776afece6fa28e23f"
HF_MODEL_REVISION="e8f8c211226b894fcb81acc59f3b34ba3efd5f42"
PADDLE_WHEEL_NAME="paddlepaddle-3.3.1-cp311-cp311-manylinux_2_35_x86_64.manylinux_2_36_x86_64.whl"
PADDLE_WHEEL="${SOURCE_DIR}/ocr_service/vendor/${PADDLE_WHEEL_NAME}"
PADDLE_WHEEL_SHA256="80d0ed4ce96859395cf634d4c3e06df4527ea2abdffc826f24b8f6bc68656f39"
...
verify_ocr_models() {
  dc --profile ocr exec -T paddleocr python -c \
    "import urllib.request; print(urllib.request.urlopen('http://127.0.0.1:8010/ready?deep=true', timeout=900).read().decode('utf-8'))"
}
...
require_safe_model_mount() {
  model_path="$1"
  model_label="$2"
  [ -n "$model_path" ] || fail "无法确定 $model_label 的 NAS 模型目录"
  case "$model_path" in
    /volume2/*) ;;
    *) fail "$model_label 模型目录不在 /volume2 下：$model_path" ;;
  esac
  [ "$model_path" != "/volume2" ] && [ "$model_path" != "/volume2/" ] \
    || fail "$model_label 模型目录范围过大"
}
...
[ -f "$SOURCE_DIR/api/catalog/management/commands/prewarm_semantic_model.py" ] \
  || fail "离线包缺少语义模型预热命令"
[ -f "$SOURCE_DIR/api/catalog/management/commands/backfill_library_processing.py" ] \
  || fail "离线包缺少存量 OCR、页码与语义 backfill 命令"
[ -f "$SOURCE_DIR/api/catalog/management/commands/configure_offline_semantic_model.py" ] \
  || fail "离线包缺少离线语义模型配置命令"
[ -f "$SOURCE_DIR/scripts/verify_meilisearch_hf_offline.sh" ] \
  || fail "离线包缺少 Meilisearch 断网模型探针"
[ -f "$SOURCE_DIR/api/catalog/migrations/0014_expand_theory_timeline_event_types.py" ] \
  || fail "离线包缺少理论系统迁移"
[ -f "$SOURCE_DIR/api/catalog/migrations/0016_alter_page_text_source.py" ] \
  || fail "离线包缺少 2.6.0 馆藏状态与页码迁移"
[ -f "$SOURCE_DIR/api/ingestion/migrations/0007_processingjob.py" ] \
  || fail "离线包缺少统一处理任务迁移"
[ -f "$SOURCE_DIR/ocr_service/model_probe.py" ] \
  || fail "离线包缺少 PaddleOCR 模型探针"
[ -s "$PADDLE_WHEEL" ] || fail "离线包缺少 N5105 无 AVX PaddlePaddle wheel"
[ -s "$HF_MODEL_ARCHIVE" ] || fail "离线包缺少固定版本 Hugging Face 模型归档"
...
[ -s "$SOURCE_DIR/web/dist/client/pdfjs/cmaps/GB-EUC-H.bcmap" ] \
  || fail "离线包缺少 PDF.js 简体中文 CMap"
[ -s "$SOURCE_DIR/web/dist/client/pdfjs/cmaps/GBK-EUC-H.bcmap" ] \
  || fail "离线包缺少 PDF.js GBK CMap"
[ -s "$SOURCE_DIR/web/dist/client/pdfjs/cmaps/Adobe-GB1-UCS2.bcmap" ] \
  || fail "离线包缺少 PDF.js Unicode 映射"
[ -s "$SOURCE_DIR/web/dist/client/pdfjs/standard_fonts/FoxitSerif.pfb" ] \
  || fail "离线包缺少 PDF.js 标准字体"
[ -f "$SOURCE_DIR/web/scripts/patch-vinext-static-paths.mjs" ] \
  || fail "离线包缺少 Vinext 静态资源路径修复"
[ -f "$SOURCE_DIR/web/scripts/prepare-pdfjs-assets.mjs" ] \
  || fail "离线包缺少 PDF.js 静态资源准备脚本"
...
tar -tzf "$HF_MODEL_ARCHIVE" >/dev/null 2>&1 || fail "Hugging Face 模型归档损坏"
hf_model_sha256="$(sha256sum "$HF_MODEL_ARCHIVE" | awk '{print $1}')"
paddle_wheel_sha256="$(sha256sum "$PADDLE_WHEEL" | awk '{print $1}')"
...
[ "$hf_model_sha256" = "$HF_MODEL_SHA256" ] || fail "Hugging Face 模型归档校验失败"
[ "$paddle_wheel_sha256" = "$PADDLE_WHEEL_SHA256" ] \
  || fail "N5105 无 AVX PaddlePaddle wheel 校验失败"
...
[ -n "$ocr_container" ] || [ "$REQUIRE_OCR_MODEL_READY" != "1" ] \
  || fail "当前 Compose 项目没有可复用的 PaddleOCR 容器"

meilisearch_container="$(dc ps -aq meilisearch)"
[ -n "$meilisearch_container" ] || fail "当前 Compose 项目没有 Meilisearch 容器"
semantic_model_host="$(docker inspect --format '{{range .Mounts}}{{if eq .Destination "/meili_data/models"}}{{println .Source}}{{end}}{{end}}' "$meilisearch_container" | head -n 1 | tr -d '\r')"
api_model_host="$(docker inspect --format '{{range .Mounts}}{{if eq .Destination "/models"}}{{println .Source}}{{end}}{{end}}' "$api_container" | head -n 1 | tr -d '\r')"
# 2.5.x only mounted the named Meilisearch volume at /meili_data. The shared
# model directory already existed on API/Worker as /models. Version 2.6.0 adds
# the same host directory at /meili_data/models, so use the live API mount as
# the upgrade source when the old Meilisearch container has no nested mount.
if [ -z "$semantic_model_host" ]; then
  semantic_model_host="$api_model_host"
elif [ -n "$api_model_host" ] && [ "$semantic_model_host" != "$api_model_host" ]; then
  fail "API 与 Meilisearch 使用了不同的 Hugging Face 模型目录"
fi
require_safe_model_mount "$semantic_model_host" "Hugging Face"
ocr_model_host=""
if [ -n "$ocr_container" ]; then
  ocr_model_host="$(docker inspect --format '{{range .Mounts}}{{if eq .Destination "/models"}}{{println .Source}}{{end}}{{end}}' "$ocr_container" | head -n 1 | tr -d '\r')"
  require_safe_model_mount "$ocr_model_host" "PaddleOCR"
fi
...
mkdir -p "$BACKUP_DIR/model-manifests"
if [ -f "$semantic_model_host/hub/library-huggingface-model-manifest.json" ]; then
  cp "$semantic_model_host/hub/library-huggingface-model-manifest.json" \
    "$BACKUP_DIR/model-manifests/huggingface-before.json"
fi
if [ -n "$ocr_model_host" ] && [ -f "$ocr_model_host/library-ocr-model-manifest.json" ]; then
  cp "$ocr_model_host/library-ocr-model-manifest.json" \
    "$BACKUP_DIR/model-manifests/paddleocr-before.json"
fi
...
SEMANTIC_MODEL_HOST=$semantic_model_host
OCR_MODEL_HOST=$ocr_model_host
EOF
...
upsert_env SEMANTIC_SEARCH_MODEL_REVISION "$HF_MODEL_REVISION" "$APP_DIR/.env"
upsert_env SEMANTIC_SEARCH_OFFLINE_MODE "true" "$APP_DIR/.env"
upsert_env HF_HUB_OFFLINE "1" "$APP_DIR/.env"
upsert_env SEMANTIC_INDEX_STAGE_BATCH_SIZE "1" "$APP_DIR/.env"
upsert_env SEMANTIC_INDEX_TASK_TIMEOUT_SECONDS "1800" "$APP_DIR/.env"
upsert_env OCR_PAGE_BATCH_SIZE "4" "$APP_DIR/.env"
upsert_env OCR_REQUIRE_PERSISTENT_MODELS "true" "$APP_DIR/.env"
if [ "$STACK_MODE" = "public-cloudflare" ]; then
...
mkdir -p "$semantic_model_host"
tar -xzf "$HF_MODEL_ARCHIVE" -C "$semantic_model_host"
hf_model_repo_cache="$semantic_model_host/hub/models--sentence-transformers--paraphrase-multilingual-MiniLM-L12-v2"
hf_model_snapshot="$hf_model_repo_cache/snapshots/$HF_MODEL_REVISION"
hf_model_revision_ref="$hf_model_repo_cache/refs/$HF_MODEL_REVISION"
[ -f "$hf_model_snapshot/model.safetensors" ] \
  || fail "Hugging Face 固定 revision 没有正确安装到 NAS 模型卷"
# Meilisearch 1.37 使用 Rust hf-hub 0.3.2。它在读取固定 commit 前仍会先查
# refs/<revision>，而 Python snapshot_download 对 commit revision 不会生成该文件。
# 缺少这个本地引用时，即使 snapshot 完整，Meilisearch 也会错误地尝试联网。
mkdir -p "$hf_model_repo_cache/refs"
printf '%s' "$HF_MODEL_REVISION" > "$hf_model_revision_ref"
[ "$(cat "$hf_model_revision_ref")" = "$HF_MODEL_REVISION" ] \
  || fail "Meilisearch 所需的 Hugging Face revision 引用没有写入"
if [ ! -e "$semantic_model_host/models--sentence-transformers--paraphrase-multilingual-MiniLM-L12-v2" ]; then
  ln -s \
    "hub/models--sentence-transformers--paraphrase-multilingual-MiniLM-L12-v2" \
    "$semantic_model_host/models--sentence-transformers--paraphrase-multilingual-MiniLM-L12-v2"
fi
meilisearch_probe_image="$(docker inspect --format '{{.Image}}' "$meilisearch_container")"
MEILISEARCH_IMAGE="$meilisearch_probe_image" \
MODEL_ROOT="$semantic_model_host" \
MODEL_REVISION="$HF_MODEL_REVISION" \
sh "$SOURCE_DIR/scripts/verify_meilisearch_hf_offline.sh" \
  || fail "固定 Hugging Face 模型没有通过断网 Meilisearch 写入和查询探针"
...
if [ -n "$ocr_target_image" ]; then
  ...
  docker run --rm --user 0:0 \
    -v "$ocr_model_host:/models" \
    --entrypoint sh "$ocr_target_image" \
    -c 'mkdir -p /models && chown -R ocr:ocr /models' \
    || fail "无法修复 PaddleOCR 模型卷权限"
  ocr_probe_args="--include-fallback --write-manifest"
  if [ "$PRELOAD_OCR_STRUCTURE" = "1" ]; then
    ocr_probe_args="$ocr_probe_args --include-structure"
  fi
  dc --profile ocr run --rm --no-deps paddleocr \
    python /app/model_probe.py $ocr_probe_args \
    || fail "PaddleOCR 模型预加载或最小推理失败，现有服务尚未重建"
fi
...
meilisearch_container="$(dc ps -q meilisearch)"
active_semantic_model_host="$(docker inspect --format '{{range .Mounts}}{{if eq .Destination "/meili_data/models"}}{{println .Source}}{{end}}{{end}}' "$meilisearch_container" | head -n 1 | tr -d '\r')"
[ "$active_semantic_model_host" = "$semantic_model_host" ] \
  || fail "升级后的 Meilisearch 没有挂载预期 Hugging Face 模型目录"
...
ocr_ready_json=""
if [ -n "$ocr_target_image" ]; then
  ocr_model_source="$(dc --profile ocr exec -T paddleocr sh -c 'printf %s "$PADDLE_PDX_MODEL_SOURCE"' | tr -d '\r\n')"
  [ "$ocr_model_source" = "modelscope" ] \
    || fail "PaddleOCR 模型源没有固定为 modelscope：$ocr_model_source"
  ocr_ready_json="$(verify_ocr_models)" || {
    dc --profile ocr logs --no-color --tail=160 paddleocr >&2 || true
    fail "PaddleOCR 服务进程没有通过真实模型加载与最小推理"
  }
fi
...
dc exec -T api python manage.py configure_offline_semantic_model \
  --repo-id sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2 \
  --revision "$HF_MODEL_REVISION" \
  --model-local-path /models \
  --dimensions 384 \
  --pooling useModel \
  --require-files \
  || fail "数据库中的语义运行配置没有切换到已校验离线 snapshot"
...
for pdfjs_resource in \
  'cmaps/GB-EUC-H.bcmap' \
  'cmaps/GBK-EUC-H.bcmap' \
  'cmaps/Adobe-GB1-UCS2.bcmap' \
  'standard_fonts/FoxitSerif.pfb'; do
  dc exec -T web sh -c "test -s /app/dist/client/pdfjs/$pdfjs_resource" \
    || fail "Web 容器缺少 PDF.js 资源：$pdfjs_resource"
done
...
if [ "$STACK_MODE" = "public-cloudflare" ]; then
  ...
  curl -fsS --connect-timeout 10 --max-time 30 \
    "http://${edge_bind_ip}:${lan_admin_port}/pdfjs/cmaps/GBK-EUC-H.bcmap" >/dev/null \
    || fail "局域网入口没有提供 PDF.js 中文 CMap"
  ...
  if [ "$VERIFY_PUBLIC_ENDPOINT" = "1" ]; then
    ...
    curl -fsS --connect-timeout 10 --max-time 30 \
      "https://${library_domain}/pdfjs/cmaps/GBK-EUC-H.bcmap" >/dev/null \
      || fail "公网域名没有提供 PDF.js 中文 CMap"
  fi
fi
...
printf '%s\n' "镜像构建没有访问 Docker Hub、npm、pip 或 Hugging Face。Hugging Face 使用已校验离线包；PaddleOCR 仅在模型卷缺失时通过现有容器网络下载。"
if [ "$PREWARM_SEMANTIC_MODEL" = "1" ]; then
  printf '%s\n' "语义模型已经在离线模式完成 Meilisearch 写入探针。"
else
