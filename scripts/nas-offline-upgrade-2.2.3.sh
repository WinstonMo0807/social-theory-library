...
BACKUP_DIR="${BACKUP_ROOT}/pre-offline-2.2.3-$(date +%Y%m%d-%H%M%S)"
API_BASE_TAG="social-theory-library-api-base:offline-pre-2.2.3"
WEB_BASE_TAG="social-theory-library-web-base:offline-pre-2.2.3"
OCR_BASE_TAG="social-theory-library-ocr-base:offline-pre-2.2.3"
...
[ -f "$SOURCE_DIR/api/catalog/migrations/0008_semantic_search_and_publication_places.py" ] \
  || fail "离线包缺少 2.2.3 数据迁移"
...
docker compose exec -T api python manage.py showmigrations catalog \
  | grep -Fq '[X] 0008_semantic_search_and_publication_places' \
  || fail "数据库迁移 0008 尚未完成"
...
printf '%s' "$health_json" \
  | grep -Eq '"version"[[:space:]]*:[[:space:]]*"2\.2\.3"' \
  || fail "健康接口没有返回版本 $EXPECTED_VERSION：$health_json"

if [ -n "$ocr_target_image" ]; then
  ocr_ready=""
  ocr_attempt=0
  while [ "$ocr_attempt" -lt 36 ]; do
    ocr_ready="$(docker compose --profile ocr ps --status running -q paddleocr 2>/dev/null || true)"
    [ -n "$ocr_ready" ] && break
    ocr_attempt=$((ocr_attempt + 1))
    sleep 5
  done
  [ -n "$ocr_ready" ] || fail "OCR 容器没有恢复运行"
fi
...
if [ -n "$ocr_target_image" ]; then
  ...
  printf '%s\n' "原部署未发现 OCR 容器，本次已更新 OCR 源码但没有启动 OCR profile。"
fi
printf '%s\n' "容器内健康检查：$health_json"
printf '%s\n' "升级前备份：$BACKUP_DIR"
printf '%s\n' "本次构建没有访问 Docker Hub、npm 或 pip。"
