...
BACKUP_DIR="${BACKUP_ROOT}/pre-theory-system-${EXPECTED_VERSION}-$(date +%Y%m%d-%H%M%S)"
...
[ -f "$SOURCE_DIR/api/accounts/migrations/0003_user_token_version.py" ] \
  || fail "离线包缺少账户会话失效迁移"
[ -f "$SOURCE_DIR/api/catalog/migrations/0012_knowledgenode_knowledgenodealias_and_more.py" ] \
  || fail "离线包缺少规范理论节点迁移"
[ -f "$SOURCE_DIR/api/catalog/migrations/0013_seed_normalized_knowledge_nodes.py" ] \
  || fail "离线包缺少旧理论映射迁移"
...
[ -f "$SOURCE_DIR/web/dist/client/runtime-config.js" ] || fail "离线包缺少 Web 运行时地址配置"
[ -f "$SOURCE_DIR/web/docker-entrypoint.sh" ] || fail "离线包缺少 Web 启动脚本"
[ -f "$SOURCE_DIR/deploy/nginx/default.conf.template" ] || fail "离线包缺少边缘代理配置"
...
[ -f "$SOURCE_DIR/ocr_service/app.py" ] || fail "离线包缺少 OCR 服务源码"
...
tar \
  ...
  --exclude='./web/dist' \
  -czf "$BACKUP_DIR/source-before-theory-system.tar.gz" \
  .
...
docker compose exec -T api python manage.py showmigrations accounts \
  | grep -Fq '[X] 0003_user_token_version' \
  || fail "账户会话失效迁移尚未完成"
docker compose exec -T api python manage.py showmigrations catalog \
  | grep -Fq '[X] 0014_expand_theory_timeline_event_types' \
  || fail "理论系统迁移尚未完成"
...
printf '%s' "$health_json" \
  | grep -Eq '"version"[[:space:]]*:[[:space:]]*"2\.3\.1"' \
  || fail "健康接口没有返回版本 $EXPECTED_VERSION：$health_json"

overview_json="$(
  docker compose exec -T api \
    curl -fsS http://127.0.0.1:8000/api/catalog/theory-system/overview/
)" || fail "规范理论系统公开接口不可用"
printf '%s' "$overview_json" | grep -Fq 'disciplines' \
  || fail "规范理论首页接口没有返回学科数据"

docker compose exec -T web \
  sh -c 'test -s /app/dist/client/runtime-config.js' \
  || fail "Web 容器没有生成运行时 API 地址配置"
if docker compose exec -T web \
  sh -c "grep -R -F 'http://localhost:8000/api' /app/dist/client/assets >/dev/null 2>&1"; then
  fail "Web 浏览器脚本仍包含错误的 localhost API 地址"
fi
...
if [ -n "$public_asset_id" ]; then
  access_json="$(
    docker compose exec -T api \
      curl -fsS "http://127.0.0.1:8000/api/distribution/assets/$public_asset_id/access/"
  )" || fail "公开 PDF 授权接口不可用"
  printf '%s' "$access_json" | grep -Fq '"url"' \
    || fail "公开 PDF 授权接口没有返回阅读地址：$access_json"
fi

printf '\n%s\n' "2.3.1 登录、PDF 阅读与理论系统 SCP 离线升级完成。"
printf '%s\n' "运行容器：API $api_version / Worker $worker_version / Web $web_version"
printf '%s\n' "容器内健康检查：$health_json"
printf '%s\n' "登录与 PDF 阅读运行时地址检查：通过"
printf '%s\n' "升级前备份：$BACKUP_DIR"
